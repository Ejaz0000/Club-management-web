...
session_start();
  ...
  $user_name = $user_data['user_name'];
	...
	$query = "select * from users where user_id IN (select user_id from club_2)";
	$result = mysqli_query($con, $query);
	$members= mysqli_fetch_all($result,MYSQLI_ASSOC);

	$qu = "select * from users where president ='app'limit 1";
	$res = mysqli_query($con, $qu);
	$pres = mysqli_fetch_assoc($res);

  $msql = "select * from message where group_id = 102 order by message_id asc";
